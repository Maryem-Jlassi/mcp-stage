...
class LLMOrchestrator:
    ...
    async def llm_orchestrate(self, user_input: str, max_pages: int = 1) -> ProcessingResult:
        ...
        # OPTIMIZED: Much shorter, structured prompt
        workflow_type = "url_workflow" if input_type == "url" else "company_workflow"
        orchestration_prompt = f"""Input: "{user_input}"
Type: {input_type}
Max pages: {max_pages}
...
{{
    ...
    "reasoning": "Using {workflow_type} for {input_type} input"
}}"""

        try:
            # OPTIMIZED: Use streaming for faster response feedback
            self.log("LLM_ORCHESTRATOR", "info", "Streaming LLM decision...")
            ...
            llm_decision = collected_response.strip()
            ...
            if decision_data is None:
                # OPTIMIZED: Faster fallback without LLM call
    ...
    async def llm_extract_jobs(self, markdown_content: str, source_url: str = "") -> List[Dict[str, Any]]:
        ...
        try:
            # OPTIMIZED: Much shorter, focused extraction prompt
            content_chunk = markdown_content[:3000]  # Reduced from 4000
            extraction_prompt = f"""Extract job listings from markdown. Return JSON array only.

Required fields: title, company, location, description, salary, requirements, contrat-type, required_skill, post_date
...
JSON:"""
            ...
            # OPTIMIZED: Use streaming for job extraction
            self.log("JOB_EXTRACTION", "info", "Streaming job extraction...")
            ...
            # OPTIMIZED: Removed second chunk processing to speed up
            if not jobs:
                self.log("JOB_EXTRACTION", "info", "No jobs found, trying fallback extraction")
...
async def main():
    if len(sys.argv) < 3:
        print("Usage: python mcp_host.py ahsen_server.py <company_name_or_url>")
        print("Examples:")
